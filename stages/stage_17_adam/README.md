...
**Builds on prior stages** — imports `Optimizer` + `SGD` (`stage_13`) and `Momentum` (`stage_16`) via `from dlfs import stage_import`, re-exports them, and ADDS only `RMSProp`, `Adam(Optimizer)`, and `AdamW(Adam)` (1st/2nd-moment EMAs + bias correction). Adam's first moment is stage_16's velocity, normalized.

**Exercise** — Implement the new optimizer classes in `code.py`, using **only** NumPy (for the array math on `p.data`/`p.grad`) and the Python stdlib. Operate on the parameter `Tensor`s from `stage_10`/`stage_11` (`p.data` and `p.grad` are NumPy arrays). Do **not** use any autodiff/optimizer library, and do **not** call `backward()` inside an optimizer — gradients are an input. The `Optimizer` base, `SGD` (both `stage_13`) and `Momentum` (`stage_16`) are imported, not re-derived.
...
**Done when**
- `pytest stage_17_adam/test.py` passes.
