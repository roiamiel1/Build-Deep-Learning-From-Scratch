...
gradient of a `Dense` layer is unchanged from `stage_10` — $\partial L/\partial
...
**Framework chain** — `code.py` imports `Dense` (`stage_10`) and `Tensor`
(`stage_08`) via `dlfs.stage_import` and uses them unchanged, adding *on top*
...
harness, using **only** NumPy, the stdlib, `Dense` from `stage_10`, and `Tensor`
from `stage_08`. Initializers return raw `np.ndarray` weights (no gradients — an
...
- `init_dense(layer, W, b=None) -> None`: overwrite `layer.W.data` (and
  `layer.b.data` if `b` is given) of a `stage_10` `Dense` **in place**, keeping
...
- Allowed tools: `numpy`, Python stdlib, `stage_10` `Dense`, `stage_08`
  `Tensor`. **No** PyTorch/autograd/etc.

**Done when**
- `pytest stage_15_weight_initialization/test.py` passes.
...
- Central-difference gradcheck of a scalar reduction of a He/Xavier-initialized
  `Dense`'s output w.r.t. `W`, `b`, and `X` matches the `stage_10` autodiff
