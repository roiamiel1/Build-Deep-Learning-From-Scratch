...
**Context** — You have a `Tensor` autodiff engine (`stage_08`, broadcast-correct backward from `stage_11`) and have just built a recurrent cell (`RNN`, `stage_26`) that mixes information across a sequence *step by step*. Self-attention mixes every position with every other position *in one shot*, with no recurrence. This is the core primitive of the Transformer; `stage_28` splits it into multiple heads and `stage_30` wraps it in a full block.
...
A **causal mask** sets $S_{ij} = -\infty$ for $j > i$ before the softmax, so position $i$ can only attend to positions $\le i$ (needed for autoregressive language models, `stage_31`); in practice add $-10^9$, not literal $-\infty$, so $\exp$ underflows to $0$ cleanly. This builds directly on the `stage_07` matmul gradients ($dL/dQ = G\,K$, $dL/dK = G^\top Q$ for $G = dL/dS$) and the `stage_12` log-sum-exp softmax. The only genuinely new gradient idea is softmax's Jacobian; for row $i$ with weights $a$ and logits $s$,
...
a "subtract the weighted mean" rule. You write **none** of these by hand: build $S$, the mask, the softmax, and $O$ entirely from `Tensor` ops (`@`, scalar multiply, `exp`, `sum(axis=, keepdims=)`, broadcast `-`/`/`) and let `Tensor.backward()` from `stage_08` (with the broadcast-correct backward of `stage_11`) flow the gradients through. Reuse the stable detached-max softmax pattern from `stage_12` so no gradient leaks through the per-row shift.
...
**Exercise** — Implement `class SelfAttention` in `code.py`. Operate on a **single sequence** of shape `(T, d_model)` (batching/multi-head is `stage_28`). Use **only** the `Tensor` from `stage_08` (loaded for you), NumPy for *forward array creation only*, and the stdlib. Do **not** hand-write any `.grad`; every gradient must flow through `Tensor.backward()`.
...
- `forward(self, x) -> Tensor`:
  ...
  - `A = softmax_rows(S)` using the detached-max + `exp` + `sum(axis=1, keepdims=True)` division pattern (numerically stable, `stage_12`).
...
- Allowed tools: `numpy` (forward only), Python stdlib, your `stage_08` `Tensor`. **No** PyTorch / TensorFlow / JAX / autograd.

**Done when**
- `pytest stage_27_self_attention/test.py` passes.
