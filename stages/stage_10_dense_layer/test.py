...
stage_08. ``Dense`` lives in this stage's ``code.py`` and is built on the
``Tensor`` imported from stage_08 through ``dlfs.stage_import``. If stage_08's
``Tensor`` or stage_10's ``Dense`` is not yet implemented, the suite skips
rather than erroring, so you can run it incrementally.

Run with:  pytest stage_10_dense_layer/test.py
"""
import os as _os
import sys as _sys
...
try:
    ...
    pytest.skip(
        f"stage_10 Dense / stage_08 Tensor not importable yet: {exc}",
