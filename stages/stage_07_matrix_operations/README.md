...
NumPy-backed `Tensor` arrives in `stage_08`.
...
$C_{ij}$ is assembled from `Value` `*`/`+`, the `stage_05` chain rule produces the two matmul
...
broadcast summing from `stage_06`.
...
**Cumulative framework** — `code.py` imports `Value` (`stage_05`) and `Vec` (`stage_06`) via
