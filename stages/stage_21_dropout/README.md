...
i.e. dropped units ($m_{ij}=0$) get zero gradient and kept units get their incoming gradient scaled by $1/p$. This is exactly `Tensor.__mul__` from `stage_08` against a freshly-sampled mask Tensor, so you reuse the engine's `_backward` rather than hand-deriving anything. The mask is resampled on every training forward pass; in eval mode you must *not* sample and *not* scale.
