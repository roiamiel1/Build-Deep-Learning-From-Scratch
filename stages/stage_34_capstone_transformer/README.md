...
`DataLoader`) and the `TransformerBlock` + hand-derived `LayerNorm` from `stage_29`
(which stacks `MultiHeadAttention` from `stage_28`). Here you wire them into a complete
...
(attention is permutation-equivariant on its own — `stage_26`), run $N$ pre-norm
`TransformerBlock`s with a **causal mask** (`causal_mask` from `stage_27`: position $t$ may
...
You do **not** hand-code its gradient — build it from `CrossEntropyLoss` (`stage_12`/`stage_31`),
...
`causal_mask` (`stage_27`), `TransformerBlock`/`LayerNorm` (`stage_29`), and
`Module`/`Parameter`/`Linear`/`Adam` (`stage_31`) via `dlfs.stage_import`; adds only the tiny
...
- `class TransformerLM(Module)` —
  ...
  from `stage_29`, a final `LayerNorm(d_model)`, and a `Linear(d_model, vocab_size)` head.
  - `forward(ids) -> Tensor`: logits of shape `(B, L, V)`; build the causal mask with
    `causal_mask(L)` (`stage_27`) and pass it to every block.
...
- `lm_loss(logits, targets) -> Tensor`: flatten `(B, L, V) -> (B*L, V)` and `(B, L) -> (B*L,)`,
  return the scalar mean `cross_entropy_loss` (`stage_12`/`stage_31`).
...
**Done when**
- `pytest stage_34_capstone_transformer/test.py` passes.
