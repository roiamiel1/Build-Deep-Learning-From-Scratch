...
Because this is built on the `Tensor` engine from `stage_08`, you do **not** hand-write gradients here — you express $z$ with `Tensor` ops (`@` for the dot product, `+` for the bias) and let `.backward()` flow through. For understanding/checking, the local derivatives are:
...
**Builds on** — imports `Tensor` from `stage_08` (via `dlfs.stage_import`) and **adds** a new `Neuron` (weighted sum + bias + activation) on top of it; `Tensor` is reused unchanged, not redefined.

**Exercise** — Implement `class Neuron` in `code.py`, using **only** `Tensor` from `stage_08` (imported with `from dlfs import stage_import`), NumPy (forward array creation only), and the stdlib. Do **not** compute any gradient by hand; all gradients must come from `Tensor.backward()`.
...
**Done when**
- `pytest stage_09_neuron/test.py` passes.
