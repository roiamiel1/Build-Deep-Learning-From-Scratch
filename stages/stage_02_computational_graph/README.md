...
**Background** — A scalar expression like $f = (a + b) \cdot c$ is really a tree of operations: a `+` node feeding a `*` node. To differentiate it automatically we must remember that structure. We extend `Value` so each node stores three things: `_prev`, the **set of input `Value`s** that produced it; `_inputs`, the **operand-ordered tuple** of those same inputs; and `_op`, a **string label** for the operation (`'+'`, `'*'`, `''` for leaves). The set is for the graph walk and dedup (`a * a` has one parent); the tuple keeps left/right order, which symmetric `+`/`*` don't need but the asymmetric `-`/`/` of later stages do — and a set can't preserve order, so we keep both. Leaves (numbers you create directly) have an empty `_prev`, empty `_inputs`, and empty `_op`. Every operator returns a *new* `Value` whose `_prev` points back at its operands, so following `_prev` from any node enumerates the subgraph that built it. This is purely bookkeeping: forward arithmetic is unchanged from stage 01, and `grad` still defaults to `0.0`. The reverse pass that will use these edges relies on the chain rule, e.g. for $L$ depending on $u = a + b$,
...
**Cumulative** — This stage imports `Value` via `stage_import("stage_01", "Value")` and SUBCLASSES it (not a rewrite). It ADDS: an `__init__` override that records graph provenance (`_inputs`, `_prev`, `_op`) on top of stage_01's `data`/`grad`, graph-aware `__add__`/`__mul__` overrides (so the whole DAG is built from this stage's `Value`), a graph-aware `__repr__`, and a module-level `trace(root)` to walk the DAG.
...
- `__init__(self, data, _inputs=(), _op='')`: OVERRIDE stage_01's `__init__`. Call `Stage1_Value.__init__(self, data)` to set `data`/`grad`, then store `self._inputs = tuple(_inputs)` (operand order), `self._prev = set(self._inputs)` (graph walk / dedup), and `self._op = _op`. stage_01's `__init__` takes only `data`, so this override is what introduces the graph fields.
- `__add__(self, other)`: accept a `Value` or a Python number; if `other` is a number, wrap it as `Value(other)`. Return `Value(self.data + other.data, (self, other), '+')` — passing operands in order so `_inputs == (self, other)` and `_prev == {self, other}`; construct THIS subclass so the whole DAG is uniform.
...
- [ ] Leaves have `_prev == set()`, `_inputs == ()` and `_op == ''`; `+`/`*` results have the right two parents and correct `_op`.
- [ ] `+`/`*` results have `_inputs == (left, right)` in operand order (so later stages can recover left/right for asymmetric ops).
