...
chain rule from `stage_05`. No NumPy autodiff: every gradient still flows through
`Value`.
...
`stage_05` ones applied per component, so `backward()` handles them
...
because the one `Value` $c$ is reused in $n$ children and `stage_05`'s `+=`
accumulation sums them. That summing-on-broadcast rule is the seed of full
tensor broadcasting in `stage_11`. The `sum()` reduction $\big(\sum_i a_i\big)$
...
`stage_05` for you (do not redefine it). You may use Python stdlib and NumPy
...
**Done when**
- `pytest stage_06_vector_operations/test.py` passes.
