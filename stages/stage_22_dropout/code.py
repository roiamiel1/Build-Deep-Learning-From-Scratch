"""Stage 22: Inverted dropout (train/eval-mode layer) on the stage_09 Tensor.
...
# Reuse prior stages: Tensor (stage_09 autodiff), MLP (stage_12, subclassed).
...
class MLPDropout(Stage12_MLP):
    """An ``MLP`` (stage_12) with a ``Dropout`` after each hidden activation (none
    ...
    # parameters() and zero_grad() are inherited from stage_12 MLP; do not override.
