...
`Dense`/`Module` conventions from `stage_15` and the regularization mindset of
`stage_21`.
...
**Cumulative chain** — imports `Tensor` (`stage_08`, via `dlfs.stage_import`) and
reuses the ones/zeros init convention (`stage_15`) and the `train()`/`eval()`
mode split (`stage_21`); **adds** a brand-new `BatchNorm1d` (hand-written
...
only) and the Python stdlib; `Tensor` is imported from `stage_08` via
...
- Implement params/grads as plain `np.ndarray` (this layer owns its backward; it
  does not flow through the `stage_08` `Tensor` engine).

**Done when**
- `pytest stage_22_batchnorm/test.py` passes.
