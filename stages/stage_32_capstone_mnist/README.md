...
`stage_31` — `Module`/`Sequential`/`Linear`/`ReLU`/`CrossEntropyLoss`/`Adam`/`DataLoader` — at the
...
$z\in\mathbb{R}^{10}$ and minimizes the mean softmax cross-entropy (the `stage_31` `CrossEntropyLoss`,
...
target test accuracy using ONLY the `stage_31` framework. Pull the whole `mytorch` API
(`Module`/`Sequential`/`Linear`/`ReLU`/`CrossEntropyLoss`/`Adam`/`DataLoader`/`Dataset`/`Tensor`) from
`stage_31` via `dlfs.stage_import`, and expose `cross_entropy_loss` as a `CrossEntropyLoss()` instance.
Define `accuracy` locally. Allowed tools: `numpy` (array math + byte parsing), `matplotlib` (optional
curves), the stdlib (`gzip`, `struct`, `os`), and the `stage_31` framework. **No PyTorch / TensorFlow /
JAX / autograd / torchvision / sklearn.** No hand-written layer gradients — every backward comes from the
imported `stage_31` layers.
...
- `make_loaders(X, y, *, batch_size, val_frac=0.0, flatten, seed=None) -> (train_loader, val_loader)`:
  `preprocess`, optional NumPy validation split, wrap each split in a `Dataset`+`DataLoader` (`stage_31`).
...
**Done when**
- `pytest stage_32_capstone_mnist/test.py` passes.
