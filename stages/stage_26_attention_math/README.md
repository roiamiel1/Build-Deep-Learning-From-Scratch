...
`stage_23` (convolution math). This is the single operation at the heart of the Transformer: a
...
**softmax / log-sum-exp** from `stage_12` (row max subtracted before exponentiating) and the matmul
gradient rules from `stage_07`: for $C=AB$ with upstream $G$, $\partial L/\partial A=G B^{\top}$ and
$\partial L/\partial B=A^{\top}G$ ("dimension analysis").

**Backward.** Let $G=\partial L/\partial O$. Through $O=AV$ (a `stage_07` matmul):
...
All arrays are plain `np.ndarray`; reuse the stable-softmax idea from `stage_12` and the matmul-grad
reasoning from `stage_07`. Support an unbatched case `(L, d)` and an optional leading batch axis
`(B, L, d)`.
...
**Done when**
- `pytest stage_26_attention_math/test.py` passes.
