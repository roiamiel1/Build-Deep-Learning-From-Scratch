...
`stage_28`), a position-wise feed-forward net (the `MLP`/`Dense` stack from `stage_11`), and the
hand-derived normalization pattern from `stage_22` (BatchNorm). This stage assembles them into the
canonical **Transformer block** — sublayer + residual + normalization, repeated twice — and derives
**LayerNorm backward by hand** (the per-token analogue of `stage_22`'s per-feature BatchNorm). One
block is the unit that gets stacked into a GPT (`stage_31`) or ViT (`stage_32`).
...
BatchNorm (`stage_22`), which standardizes each feature **across the batch**, LayerNorm standardizes
...
This is `stage_22`'s collapsed formula with the reduction axis moved from batch to features.
...
`stage_28` and the `MLP` from `stage_11`, both imported through `dlfs.stage_import`. **No
autodiff library.** Like `stage_22`, the norm owns explicit `forward`/`backward` over raw
...
- `TransformerBlock(d_model, n_heads, d_ff, *, norm="pre", eps=1e-5, seed=None)`:
  - Holds `attn` (a `MultiHeadAttention(d_model, n_heads)` from `stage_28`), two `LayerNorm(d_model)`
    (`ln1`, `ln2`), and a position-wise `ffn` = `MLP([d_model, d_ff, d_model], activation="relu")`
    from `stage_11`. `norm` is `"pre"` or `"post"`.
...
**Done when**
- `pytest stage_29_transformer/test.py` passes.
