...
**Context** — You have a working `TransformerBlock` (`stage_29`: residual MHA +
FFN + hand-derived `LayerNorm`) and the `im2col` patch-unfolding machinery
(`stage_24`). A **Vision Transformer (ViT)** drops the convolutional inductive
...
you reuse `im2col` from `stage_24` to extract the patches. A learnable
...
is fed through $L$ `TransformerBlock`s (`stage_29`); a final `LayerNorm` is
...
`stage_07`/`stage_10`: for the patch embed $Z=\mathrm{cols}\,E+b$,
...
NumPy `forward`/`backward` (the `stage_26`–`stage_29` paradigm: parameters are
`np.ndarray`, backward is hand-written from a forward cache). Reuse `im2col`
(`stage_24`), `TransformerBlock` + `LayerNorm` (`stage_29`) via `dlfs.stage_import`.
...
**Done when**
- `pytest stage_30_vision_transformer/test.py` passes.
